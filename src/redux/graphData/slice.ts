...
const userSlice = createSlice({
	...
	reducers: {
		deleteFavorite: (state, { payload }) => {
			const newState = state.user.favorites.filter((favorite) => favorite !== payload);
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(editPhotoThunk.fulfilled, (state, { payload }) => {
				state.user.avatar = `https://res.cloudinary.com/dddrrdx7a/image/upload/v1707757640/${payload}` as string;
			...
			.addMatcher(isAnyOf(getUserThunk.pending, editUserThunk.pending), (state) => {
			...
			.addMatcher(
				...
				(state, { payload }) => {
					state.isLoadingUser = false;
					state.isLoadingAvatar = false;
					state.error = typeof payload === "string" ? payload : "Unknown type of error occurred";
				}
			);
	},
});
